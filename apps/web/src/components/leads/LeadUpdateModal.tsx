...
import { 
  ...
  shouldShowGuidedWorkflow,
  getOffRampRequirements,
  canRevertStatus,
  validateCallSummary, 
  validateTrialScheduling, 
...
// SUBSCRIPTION_PLANS and calculateSubscriptionEndDate now imported from @/lib/logic/subscriptions
...
function AttendanceSummary({ leadId }: AttendanceSummaryProps) {
  ...
  return (
    <div className="p-5 bg-white border-2 border-green-200 rounded-2xl">
      <h3 className="text-sm font-black text-gray-700 uppercase tracking-widest mb-4 font-bebas">
        📊 Attendance Summary
      </h3>
...
export function LeadUpdateModal({ lead, isOpen, onClose, onJoined }: LeadUpdateModalProps) {
  ...
  const whatsappMessage = useMemo(() => {
    ...
    return formatMessage(brandConfig.messages.preferenceLink, {
      playerName: lead.player_name,
      academyName: brandConfig.name,
      preferenceLink: prefUrl,
    });
  }, [lead]);
  ...
  const effectiveStatus = calculateEffectiveStatus(currentStatus, hasParentPreferences);

  // Determine which step to show based on current status
  const showGuidedWorkflow = shouldShowGuidedWorkflow(currentStatus);
  ...
  const handleSendPreferenceLink = async () => {
    ...
    const whatsappUrl = generateWhatsAppLink(lead.phone, whatsappMessage, {
      playerName: lead.player_name,
      academyName: brandConfig.name,
      preferenceLink: prefUrl,
    });
  ...
  const handleConfirmCall = async () => {
    const validation = validateCallSummary(callSummary);
    if (!validation.isValid || !callConfirmed) {
      toast.error(validation.error || 'Please confirm you called');
      return;
    }
  ...
  const handleRevertStatus = async () => {
    ...
    if (!canRevertStatus(currentStatus)) {
        ...
        <div className="p-8 overflow-y-auto space-y-8 flex-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-brand-accent/10 p-5 rounded-2xl border border-brand-accent/20">
              <p className="text-[10px] text-brand-primary font-bold uppercase tracking-widest mb-1">Contact Phone</p>
              ...
              <div className="space-y-3">
                ...
                {lead.preferred_call_time && (
                  <div>
                    ...
                    <p className="text-sm font-bold text-brand-primary">{lead.preferred_call_time}</p>
                  </div>
                )}
                {lead.preferred_timing_notes && (
                  <div>
                    <p className="text-xs font-semibold text-gray-600 mb-1">Additional Notes:</p>
                    <p className="text-sm text-brand-primary whitespace-pre-wrap">{lead.preferred_timing_notes}</p>
                  </div>
                )}
              </div>
            </div>
              ...
              {currentStatus === 'New' && !hasParentPreferences && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 mb-4">
                    <div className="flex items-center justify-center w-10 h-10 rounded-full bg-brand-accent/20 text-brand-primary font-bold">
                      1
                    </div>
                    <div className="flex-1">
                      <h3 className="text-[10px] font-semibold text-slate-500 uppercase tracking-[0.2em]">Send Preference Link</h3>
              ...
              {(effectiveStatus === 'Followed up with message') && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <div className="flex-1">
                        <h3 className="text-[10px] font-semibold text-slate-500 uppercase tracking-[0.2em]">Log Confirmation Call</h3>
                        <div className="mt-1 h-px bg-gray-200"></div>
                      </div>
                      {hasParentPreferences && currentStatus === 'New' && (
                        <p className="text-xs text-emerald-600 mt-1">✓ Parent preferences received - skipping Step 1</p>
                      )}
                    </div>
                  </div>
                  
                  <div className="space-y-4 p-5 bg-brand-accent/10 border border-brand-accent/20 rounded-2xl">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input
                        ...
                        disabled={!canEdit}
                        className="w-5 h-5 rounded border-gray-300 text-brand-accent focus:ring-brand-accent disabled:opacity-50"
                      />
              ...
              {currentStatus === 'Called' && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-[10px] font-semibold text-slate-500 uppercase tracking-[0.2em]">Schedule Trial</h3>
                      <div className="mt-1 h-px bg-gray-200"></div>
                    </div>
                  </div>
                    ...
                    {isReturningSoon && lead.phone && (
                      <button
                        onClick={() => {
                          ...
                          const message = formatMessage(brandConfig.messages.welcomeBackNudge, {
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          const whatsappMessage = formatMessage(brandConfig.messages.reengagementNudge, {
          ...
          {currentStatus === 'Joined' && studentRecord && (
            <div className="space-y-6 border-t-4 border-green-500 pt-6">
              ...
              <div className="p-3 sm:p-5 bg-gradient-to-br from-brand-accent/10 via-brand-accent/5 to-brand-surface border-2 border-brand-accent/30 rounded-2xl overflow-y-auto max-h-[400px] sm:max-h-[600px]" data-section="progress-hub">
                <h3 className="text-xs sm:text-sm font-black text-brand-primary uppercase tracking-widest mb-4 flex items-center gap-2 font-bebas">
                    ...
                    <div className="space-y-3 sm:space-y-4">
                      ...
                      {latestReport && (
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p className="text-xs sm:text-sm font-bold text-brand-primary uppercase tracking-wide">
                            🏆 Latest Achievement
                          </p>
                        </div>
                      )}
                            ...
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3 sm:mb-4">
                              ...
                              <div className="text-left sm:text-right">
                                {latestReport.milestone_label && (
                                  <p className="text-[10px] sm:text-xs font-bold text-brand-primary bg-brand-accent/20 px-2 py-0.5 sm:py-1 rounded-full mb-1 inline-block">
                                    {latestReport.milestone_label}
                                  </p>
                                )}
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 mb-1.5 sm:mb-1">
                                      ...
                                      {report.milestone_label && (
                                        <span className="text-[10px] sm:text-xs font-bold text-brand-primary bg-brand-accent/20 px-2 py-0.5 rounded-full whitespace-nowrap">
                                          {report.milestone_label}
                                        </span>
                                      )}
                                    </div>
                ...
                <div className="p-5 bg-white border-2 border-gray-200 rounded-2xl">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <button
                      ...
                      className="flex items-center justify-center gap-2 p-4 bg-brand-accent/10 border-2 border-brand-accent/30 rounded-xl hover:bg-brand-accent/20 transition-colors"
                    >
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try {
                                ...
                                const message = formatMessage(brandConfig.messages.gracePeriodActivated, {
                                  playerName: playerName,
                                });
                                const whatsappUrl = generateWhatsAppLink(lead.phone, message, {});
                                window.open(whatsappUrl, '_blank');
                          ...
                          <button
                            onClick={async () => {
                              ...
                              try {
                                ...
                                const playerName = studentRecord.lead_player_name || lead.player_name;
                                const message = formatMessage(brandConfig.messages.gracePeriodFinalReminder, {
                                  playerName: playerName,
                                });
                                const whatsappUrl = generateWhatsAppLink(lead.phone, message, {});
                                window.open(whatsappUrl, '_blank');
