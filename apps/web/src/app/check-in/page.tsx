...
function CheckInPageContent() {
  ...
  const allMarked = isAllAttendanceMarked(attendanceSummary);
  ...
  const handleAttendanceClick = async (
    ...
    try {
      // Use shared logic to build attendance payload
      const payload = buildAttendancePayload(participantType, studentId, leadId, batchId, status);
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-0.5">
                                ...
                                {(() => {
                                  ...
                                  const attendanceRecords: AttendanceRecord[] = history.attendance.map((r: any) => ({
                                    date: r.date,
                                    status: r.status as 'Present' | 'Absent',
                                  }));
                                  
                                  const indicator = getAttendanceStatusIndicator(attendanceRecords);
                                  if (!indicator) return null;
                                  
                                  return (
                                    <span className={`text-[10px] font-bold ${indicator.color}`}>
